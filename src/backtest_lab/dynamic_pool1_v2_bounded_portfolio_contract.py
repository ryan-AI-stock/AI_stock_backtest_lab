...
def run_dynamic_pool1_v2_bounded_portfolio_contract(
    *,
    ...
    member_panel = _load_member_panel(panel_path)
    ...
    signal_panel = _build_daily_signal_panel(formal, monthly_candidates, trading_calendar)
...
def _load_member_panel(path: Path) -> pd.DataFrame:
    if not path.exists():
    ...
    return df.dropna(subset=["candidate_as_of_date", "candidate_rank"]).copy()
...
def _build_cost_ledger(weight_ledger: pd.DataFrame) -> pd.DataFrame:
    ...
    out = weight_ledger[["date", "next_tradable_date", "dynamic_pool_variant", "ticker", "dynamic_weight"]].copy()
