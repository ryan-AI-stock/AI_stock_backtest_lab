...
def run() -> None:
    ...
    (OUT/"final_summary_zh.md").write_text("# P3 Layer0-4 canonical rank1 weighted timing contract\n\n本contract是使用者明確授權的bounded rank1 timing subproblem：只評估canonical weekly rank1的stock/market/combined entry-exit timing。715日與5/10/20/40TD labels已鎖定；P3-1三fold校準，P3-2一次untouched OOS。它不執行all80 rerank，也不能回答完整Layer5 selector成敗。Stage A ready；未產NAV、Top3或正式交易決策。\n",encoding="utf-8")
