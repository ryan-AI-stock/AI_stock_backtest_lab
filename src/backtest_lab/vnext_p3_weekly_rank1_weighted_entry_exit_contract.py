...
OUT = ROOT / "outputs/vnext_p3_layer5_weekly_rank1_stock_market_weighted_entry_exit_contract_20260713"
TASK = "TASK-BACKTEST-CORE-VNEXT-P3-LAYER5-WEEKLY-RANK1-STOCK-MARKET-WEIGHTED-ENTRY-EXIT-CONTRACT-001"
...
def run() -> None:
    ...
    ready=len(frame)==715 and frame.decision_date.nunique()==715 and len(folds)==3 and labels.loc[labels.horizon_td.isin([10,20]),"label_mature_and_ready"].sum()>0
    readiness={"task_id":TASK,"status":"weekly_rank1_weighted_entry_exit_contract_ready_for_stage_A" if ready else "blocked","requested_start":"2023-07-11","requested_end":"2026-06-29","actual_start":str(frame.decision_date.min().date()),"actual_end":str(frame.decision_date.max().date()),"daily_rank1_rows":len(frame),"decision_dates":frame.decision_date.nunique(),"P3_1_fold_count":len(folds),"P3_2_untouched_OOS":True,"parameter_effective_degrees_freedom":17,"non_cartesian_calibration":True,"stock_and_exit_scores_separate":True,"old_hardcoded_thresholds_used":False,"normal_switch_enabled":False,"portfolio_NAV_materialized":False,"ready_for_stage_A_candidate_quality":bool(ready),"ready_for_experiments":bool(ready),"ready_for_stage_B_NAV":False,"future_data_violation_count":0,"formal_model_changed":False,"trade_decision_changed":False,"active_in_trade_decision":False,"report_changed":False,"portfolio_replay_executed":False,"ready_for_strategy_replay":False,"ready_for_formal":False,"not_live_rule":True,"forward_returns_live_rule_usage":False}
    ...
    (OUT/"final_summary_zh.md").write_text("# P3 weekly rank1 stock-market weighted entry/exit contract\n\n本次只使用canonical Layer0-4 weekly rank1作候選，daily更新個股entry、獨立exit deterioration與full_spec_v2五群market risk。715個decision dates與5/10/20/40TD labels已鎖定。P3-1三fold校準，P3-2一次untouched OOS；不沿用舊2日/5分/0.7門檻。Stage A only，未產NAV、未啟用normal switch。\n",encoding="utf-8")
