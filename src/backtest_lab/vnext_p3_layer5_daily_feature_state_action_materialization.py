...
def run():
    ...
    # Market groups: preserve simple_v0 and materialize frozen full_spec_v1.
    ...
    market['simple_v0_taiwan_group']=np.select([(market.adj_close>market.ma60)&(market.ret20>=0),(market.adj_close<market.ma60)&(market.ret20<0)],['bullish','bearish'],'neutral')
    ...
    advance=x.sort_values(['ticker','decision_date']).assign(advance=lambda q:q.groupby('ticker').adjusted_close.pct_change()>0).groupby('decision_date').advance.mean()
    market=market.merge(bx,on='decision_date',how='left').merge(advance.rename('advance_share'),on='decision_date',how='left'); market['simple_v0_breadth_group']=np.select([(market.above>=.6)&(market.rspos>=.6),(market.above<=.4)&(market.rspos<=.4)],['bullish','bearish'],'neutral')
    ...
    market['simple_v0_capital_group']=np.select([(market.turnover>market.turnover.rolling(20,min_periods=10).mean())&(market.margin.diff(20)<=0),(market.turnover<market.turnover.rolling(20,min_periods=10).mean())&(market.margin.diff(20)>0)],['bullish','bearish'],'neutral')
    tf=load_years(RADAR,'taifex'); tf['date']=pd.to_datetime(tf.date); tf=tf.groupby('date',as_index=False).foreign_futures_oi_net_contracts.sum(); tf['chg20']=tf.foreign_futures_oi_net_contracts.diff(20); market=market.merge(tf[['date','foreign_futures_oi_net_contracts','chg20']],left_on='decision_date',right_on='date',how='left').drop(columns='date'); market['simple_v0_derivatives_group']=np.select([market.chg20>0,market.chg20<0],['bullish','bearish'],'neutral')
    sox=pd.read_csv(MARKET/'global_market/p3_sox.csv.gz'); sox['session_date']=pd.to_datetime(sox.session_date); sox=sox.sort_values('session_date'); sox['sox20']=sox.adjusted_close.pct_change(20); market=pd.merge_asof(market.sort_values('decision_date'),sox[['session_date','sox20']],left_on='decision_date',right_on='session_date',direction='backward'); market['simple_v0_external_group']=np.select([market.sox20>0,market.sox20<-.05],['bullish','bearish'],'neutral')
    simple_groups=[f'simple_v0_{g}_group' for g in ['taiwan','breadth','capital','derivatives','external']]; market['simple_v0_bullish_count']=(market[simple_groups]=='bullish').sum(axis=1); market['simple_v0_bearish_count']=(market[simple_groups]=='bearish').sum(axis=1)
    sr=np.select([(market.simple_v0_bearish_count>=4)&(market.simple_v0_taiwan_group=='bearish')&(market.simple_v0_breadth_group=='bearish'),market.simple_v0_bearish_count>=3,(market.simple_v0_bullish_count>=3)&(market.simple_v0_bearish_count<=1)],['bear_candidate','weak_market','strong_market'],'ordinary_market'); market['simple_v0_market_state']=sr; sb=pd.Series(sr).eq('bear_candidate'); market.loc[sb&sb.shift(1,fill_value=False),'simple_v0_market_state']='confirmed_bear'; market.loc[market.simple_v0_market_state=='bear_candidate','simple_v0_market_state']='weak_market'

    def vote(cols):
        bull=(market[cols]>0).sum(axis=1); bear=(market[cols]<0).sum(axis=1); avail=market[cols].notna().sum(axis=1)
        return np.select([(bull>bear)&(bull>=2),(bear>bull)&(bear>=2)],['bullish','bearish'],'neutral'),avail/len(cols)
    market['0050_above20']=np.sign(market.adj_close-market.ma20); market['0050_above60']=np.sign(market.adj_close-market.ma60); market['0050_slope20']=np.sign(market.ma20.diff(5)); market['0050_slope60']=np.sign(market.ma60.diff(5)); market['0050_ret20_dir']=np.sign(market.ret20)
    tw=pd.read_csv(ROOT/'backtest_cache/taiex_yfinance/^TWII.csv'); tw['date']=pd.to_datetime(tw.date); tw=tw.sort_values('date'); tw['tw_ma20']=tw.adj_close.rolling(20).mean(); tw['tw_ma60']=tw.adj_close.rolling(60).mean(); tw['tw_ret20']=tw.adj_close.pct_change(20); tw['tw_above20']=np.sign(tw.adj_close-tw.tw_ma20); tw['tw_above60']=np.sign(tw.adj_close-tw.tw_ma60); tw['tw_slope20']=np.sign(tw.tw_ma20.diff(5)); tw['tw_slope60']=np.sign(tw.tw_ma60.diff(5)); tw['tw_ret20_dir']=np.sign(tw.tw_ret20)
    market=market.merge(tw[['date','tw_above20','tw_above60','tw_slope20','tw_slope60','tw_ret20_dir']],left_on='decision_date',right_on='date',how='left').drop(columns='date')
    tai_cols=['0050_above20','0050_above60','0050_slope20','0050_slope60','0050_ret20_dir','tw_above20','tw_above60','tw_slope20','tw_slope60','tw_ret20_dir']; market['taiwan_group'],market['taiwan_confidence']=vote(tai_cols)
    market['breadth_above_dir']=np.sign(market.above-.5); market['breadth_rs_dir']=np.sign(market.rspos-.5); market['breadth_advance_dir']=np.sign(market.advance_share-.5); market['breadth_group'],market['breadth_confidence']=vote(['breadth_above_dir','breadth_rs_dir','breadth_advance_dir'])
    market['market_ret']=market.adj_close.pct_change(); market['market_vol20']=market.market_ret.rolling(20).std(); market['market_vol60']=market.market_ret.rolling(60).std(); market['capital_turnover_dir']=np.sign(market.turnover-market.turnover.rolling(20,min_periods=10).mean()); market['capital_margin_dir']=-np.sign(market.margin.diff(20)); market['capital_vol_dir']=-np.sign(market.market_vol20-market.market_vol60); market['capital_group'],market['capital_confidence']=vote(['capital_turnover_dir','capital_margin_dir','capital_vol_dir'])
    market['deriv_level_dir']=np.sign(market.foreign_futures_oi_net_contracts); market['deriv_change_dir']=np.sign(market.chg20); market['deriv_context_dir']=np.sign(market.foreign_futures_oi_net_contracts.rolling(20,min_periods=10).mean()); market['derivatives_group'],market['derivatives_confidence']=vote(['deriv_level_dir','deriv_change_dir','deriv_context_dir'])
    gm=load_years(RADAR,'global_market'); gm['session_date']=pd.to_datetime(gm.session_date)
    for field,key,invert in [('Nasdaq','nasdaq',False),('VIX','vix',True),('USD_TWD','usdtwd',True)]:
        z=gm[gm.field==field].sort_values('session_date').copy(); z[key+'_20']=z.adjusted_close.pct_change(20); market=pd.merge_asof(market.sort_values('decision_date'),z[['session_date',key+'_20']],left_on='decision_date',right_on='session_date',direction='backward',suffixes=('','_'+key)); market[key+'_dir']=(-1 if invert else 1)*np.sign(market[key+'_20'])
    market['sox_dir']=np.sign(market.sox20); market['external_group'],market['external_confidence']=vote(['nasdaq_dir','sox_dir','vix_dir','usdtwd_dir'])
    ...
    raw=np.select([(market.bearish_count>=4)&(market.taiwan_group=='bearish')&(market.breadth_group=='bearish'),market.bearish_count>=3,(market.bullish_count>=3)&(market.bearish_count<=1)],['bear_candidate','weak_market','strong_market'],'ordinary_market'); market['full_spec_v1_market_state']=raw; bear=pd.Series(raw).eq('bear_candidate'); market.loc[bear&bear.shift(1,fill_value=False),'full_spec_v1_market_state']='confirmed_bear'; market.loc[market.full_spec_v1_market_state=='bear_candidate','full_spec_v1_market_state']='weak_market'
    market['market_state']=market.full_spec_v1_market_state; market['controller_version']='full_spec_v1'; market['market_confidence']=market[['taiwan_confidence','breadth_confidence','capital_confidence','derivatives_confidence','external_confidence']].mean(axis=1)
    x=x.merge(market[['decision_date']+groups+['bullish_count','bearish_count','market_state','market_confidence','controller_version','simple_v0_market_state']],on='decision_date',how='left')
    ...
    md=market[['decision_date','simple_v0_market_state','full_spec_v1_market_state']].copy(); md['state_changed']=md.simple_v0_market_state.ne(md.full_spec_v1_market_state); md['simple_transition']=md.simple_v0_market_state.ne(md.simple_v0_market_state.shift()); md['v1_transition']=md.full_spec_v1_market_state.ne(md.full_spec_v1_market_state.shift()); md.to_csv(OUT/'p3_layer5_market_simple_v0_vs_full_spec_v1_diff.csv',index=False,encoding='utf-8-sig')
