...
def direction(z,thr=.5):
 out=pd.Series(np.select([z>=thr,z<=-thr],[1.,-1.],0.),index=z.index,dtype=float)
 out[pd.isna(z)]=np.nan
 return out
...
def run():
 ...
 m['deriv_level_crowding_warning']=(zroll(m.oi_contracts).abs()>=1.5)|(zroll(m.oi_amount).abs()>=1.5); m['deriv_level']=np.where(m.oi_contracts.notna()&m.oi_amount.notna(),0.,np.nan)
 ...
 market5=(m.etf_ret5+m.taiex_ret5)/2; oi5=m.oi_contracts.diff(5); div_base=direction(zroll(m.oi_contracts.diff(5)-m.oi_contracts.diff(20))); m['deriv_divergence']=pd.Series(np.select([(market5>0)&(oi5<0),(market5<0)&(oi5>0)],[-1.,1.],div_base),index=m.index); m.loc[market5.isna()|oi5.isna(),'deriv_divergence']=np.nan
 ...
 m['external_nasdaq']=direction((zroll(m.nasdaq_ret20)+zroll(m.nasdaq_ret60)+zroll(m.nasdaq_slope20))/3); m['external_sox']=direction((zroll(m.sox_ret20)+zroll(m.sox_ret60)+zroll(m.sox_slope20))/3); m['external_vix']=direction(-((zroll(m.vix_ret5)+zroll(m.vix_ret20)+zroll(m.vix_pct-.5))/3)); m['external_fx']=direction(-((zroll(m.fx_ret5)+zroll(m.fx_ret20)+zroll(m.fx_ret60))/3)); m['external_crossconfirm']=pd.Series(np.select([(m.external_nasdaq>0)&(m.external_sox>0)&(m.external_vix>=0),(m.external_nasdaq<0)&(m.external_sox<0)&(m.external_vix<=0)], [1.,-1.],0.),index=m.index); m.loc[m[['external_nasdaq','external_sox','external_vix']].isna().any(axis=1),'external_crossconfirm']=np.nan; m['fx_export_vs_outflow_conflict']=(m.external_fx<0)&((m.external_nasdaq>0)|(m.external_sox>0))
 ...
 groups=['taiwan_group','breadth_group','capital_group','derivatives_group','external_group']; confcols=['taiwan_confidence','breadth_confidence','capital_confidence','derivatives_confidence','external_confidence']; m['bullish_groups']=(m[groups]=='bullish').sum(axis=1); m['bearish_groups']=(m[groups]=='bearish').sum(axis=1); m['high_confidence_group_count']=(m[confcols]>=.6).sum(axis=1); candidate=(m.bearish_groups>=4)&m.taiwan_group.eq('bearish')&m.breadth_group.eq('bearish'); m['full_spec_v2_state']=np.select([candidate,m.bearish_groups>=3,(m.bullish_groups>=3)&(m.bearish_groups<=1)],['bear_candidate','weak_market','strong_market'],'ordinary_market'); m.loc[candidate&candidate.shift(1,fill_value=False),'full_spec_v2_state']='confirmed_bear'; m.loc[m.full_spec_v2_state=='bear_candidate','full_spec_v2_state']='weak_market'; m['controller_state_status']=np.where(m.high_confidence_group_count>=3,'ready','PIT_warmup_or_low_confidence'); m.loc[m.controller_state_status!='ready','full_spec_v2_state']='PIT_warmup_or_low_confidence'; m['controller_version']='full_spec_v2'
 ...
 readiness['warmup_low_confidence_rows']=int((m.controller_state_status!='ready').sum()); readiness['warmup_NA_preserved']=True
 (OUT/'readiness_for_p3_market_controller_full_spec_v2.json').write_text(json.dumps(readiness,ensure_ascii=False,indent=2),encoding='utf-8'); (OUT/'final_summary_zh.md').write_text('# P3 market controller full_spec_v2\n\n五群組已按direction/magnitude/persistence/reversal維度實算；TAIFEX trade/OI四欄皆ready並依next-trading-date PIT使用。Rolling warmup保留NA，低信心日明標PIT_warmup_or_low_confidence。未跑績效。\n',encoding='utf-8')
