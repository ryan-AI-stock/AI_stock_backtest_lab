...
class StockPoolObservationTest(unittest.TestCase):
    ...
    def test_batch_writes_manifest_and_skips_empty_pool(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("cost_model_boundary", manifest)
            self.assertEqual(manifest["cost_model_boundary"]["cost_model_version"], "taiwan_standard_fee_tax_v1")
            self.assertFalse(manifest["cost_model_boundary"]["daily_observation_profit_fields_present"])
            self.assertTrue(manifest["cost_model_boundary"]["formal_replay_cost_deducted_when_profit_is_reported"])
            ...
            self.assertTrue(wording["formal_model_changed"])
            self.assertTrue(wording["trade_decision_changed"])
            self.assertTrue(wording["active_in_trade_decision"])
            ...
            self.assertTrue(wording["execution_boundary"]["active_in_trade_decision"])
            self.assertIn("非正式診斷", wording["diagnostic_boundary"]["description"])
            self.assertIn("風險控管空手", wording["execution_boundary"]["description"])
            self.assertTrue(manifest["formal_report_ready"])
            self.assertEqual(manifest["formal_report_blocker_count"], 0)
    ...
    def test_decision_first_contract_does_not_promote_observation_candidate_to_formal_target(self) -> None:
        ...
        self.assertEqual(decision["decision_first_state"], "risk_control_cash")
        self.assertEqual(decision["action_state"], "risk_control_cash")
        self.assertEqual(decision["action_state_zh"], "風險控管空手")
        self.assertEqual(decision["formal_target_display"], "風險控管空手 / 現金")
        self.assertEqual(decision["formal_target_ticker"], "CASH")
        self.assertIn("模型未找到合格攻擊標的", decision["operation_reason_zh"])
        ...
        visible = json.dumps(decision, ensure_ascii=False)
        self.assertNotIn("no formal target", visible)
        self.assertNotIn("target_weights={}", visible)
        self.assertNotIn("bug", visible)
    ...
    def test_batch_report_hides_0050_candidate_and_repeated_code_display(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("pool1_pool2_confirmation1_base", report)
            self.assertNotIn("pool1_primary_pool2_confirmation", report)
            self.assertNotIn("上限40", report)
            self.assertNotIn("上限 40", report)
            self.assertNotIn("最高持倉", report)
