...
class MaSignalLeveragedEtfBacktestTests(unittest.TestCase):
    def test_entry_signal_supports_seven_and_ten_day_windows(self) -> None:
        dates = pd.date_range("2020-01-01", periods=10, freq="B")
        frame = pd.DataFrame({"date": dates, "0050_adj_close": range(1, 11), "00631L_adj_close": [20] * 10})
        for entry_window in (7, 10):
            with self.subTest(entry_window=entry_window):
                signaled = add_signals(frame, entry_window, 3)
    ...
    def test_signal_executes_on_next_trading_day(self) -> None:
        ...
        result = simulate(frame, period={"period": "T", "requested_start": "2020-01-01", "requested_end": "2020-01-31"}, entry_window=5, exit_window=3, after_cost=False)
