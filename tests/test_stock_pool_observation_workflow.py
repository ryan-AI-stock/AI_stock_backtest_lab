...
class StockPoolObservationWorkflowTest(unittest.TestCase):
    def test_workflow_checks_out_radar_repo_and_passes_formal_radar_data_dir(self) -> None:
        ...
        self.assertIn("Refresh TWSE institutional flow data", workflow)
        self.assertIn("python -m backtest_lab.twse_institutional_flow_refresh", workflow)
        self.assertIn("--status-json \"$status_path\"", workflow)
        self.assertIn("institutional_flows.status.json", workflow)
        self.assertIn("institutional_flow_status=$status_path", workflow)
        self.assertIn("steps.institutional-flow-refresh.outputs.institutional_flow_data", workflow)
        self.assertIn("--require-fresh-institutional-flow", workflow)
        ...
        self.assertIn("stock_pool_observation_manifest.json", workflow)
        self.assertIn("formal_report_ready", workflow)
        self.assertIn("actual_signal_date", workflow)
